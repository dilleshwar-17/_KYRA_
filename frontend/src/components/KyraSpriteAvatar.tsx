...
const KyraSpriteAvatar: React.FC<KyraSpriteAvatarProps> = ({ 
    ...
    useEffect(() => {
        ...
        const getSpriteInfo = (imgKey: string, oy: number) => {
            const img = imagesRef.current[imgKey] || imagesRef.current['neutral'];
            ...
            const dx = (CW - dw) / 2;
            const dy = CH - dh - 5 + oy;
            return { img, dx, dy, dw, dh, s };
        };

        const setupTransform = (info: any, tiltDeg: number, scaleY: number) => {
            ctx.save();
            // Pivot is now relative to the sprite's face area
            const px = CW / 2;
            const py = info.dy + info.dh * 0.22; 
            ctx.translate(px, py);
        ...
        const drawBase = (info: any) => {
            ctx.drawImage(info.img, 0, 0, info.img.naturalWidth, info.img.naturalHeight, info.dx, info.dy, info.dw, info.dh);
        };

        const drawBlink = (info: any, amount: number) => {
            if (amount <= 0) return;
            ctx.fillStyle = '#b4845a';
            const ey = info.dy + info.dh * 0.20;
            const eh = info.dh * 0.035 * amount;
            const ew = info.dw * 0.15;
            [[info.dx + info.dw * 0.32, ey], [info.dx + info.dw * 0.58, ey]].forEach(([ex]) => {
                ctx.beginPath();
        ...
        const drawProceduralFace = (info: any, expression: string, mouthOpenness: number) => {
            const { dx, dy, dw, dh } = info;
            ...
            if (mouthOpenness > 0) {
                ...
                if (expression === 'surprised') {
                    ctx.beginPath(); ctx.ellipse(mouthX, mouthY + oh/2 + 2, mouthW/2.2, oh + 2, 0, 0, Math.PI * 2); ctx.fill();
                } else if (expression === 'sad') {
                    ctx.beginPath(); ctx.ellipse(mouthX, mouthY + oh/2 + 3, mouthW/2, oh/1.5 + 1, 0, 0, Math.PI * 2); ctx.fill();
                } else {
                    ctx.beginPath(); ctx.ellipse(mouthX, mouthY + oh/2 + 1, mouthW/2, oh/2 + 1, 0, 0, Math.PI * 2); ctx.fill();
                    ctx.fillStyle = 'rgba(215,115,100,0.6)';
                    ctx.beginPath(); ctx.ellipse(mouthX, mouthY + oh * 0.35 + 1, mouthW/2 * 0.8, oh * 0.35, 0, 0, Math.PI); ctx.fill();
                }
            } else if (expression !== 'neutral') {
                ctx.strokeStyle = '#a65d4f'; ctx.lineWidth = 2.5; ctx.beginPath();
                if (expression === 'sad') {
                    ctx.moveTo(mouthX - mouthW/2, mouthY + 7); ctx.quadraticCurveTo(mouthX, mouthY + 2, mouthX + mouthW/2, mouthY + 7);
                } else if (expression === 'happy') {
                    ctx.moveTo(mouthX - mouthW/1.8, mouthY + 2); ctx.quadraticCurveTo(mouthX, mouthY + 9, mouthX + mouthW/1.8, mouthY + 2);
                } else if (expression === 'thinking') {
                    ctx.moveTo(mouthX - mouthW/4, mouthY + 4); ctx.lineTo(mouthX + mouthW/4, mouthY + 4);
                }
                ctx.stroke();
            }
        };

        const drawGlow = (color: string, intensity: number, oy: number) => {
            const g = ctx.createRadialGradient(CW / 2, CH * 0.4 + oy, 5, CW / 2, CH * 0.4 + oy, 110);
        ...
        const render = (ts: number) => {
            ...
            if (currentState === 'idle') {
                ...
                const info = getSpriteInfo(currentExpr, sentimentShift);
                if (info) {
                    setupTransform(info, 0, sc);
                    drawBase(info);
                    drawProceduralFace(info, currentExpr, 0);
                    drawBlink(info, tickBlink(dt));
                    ctx.restore();
                }
                ...
                d.talkTimer += dt;
                if (d.talkTimer > d.talkSpeed) {
                    d.talkTimer = 0;
                    d.talkIdx = (d.talkIdx + 1) % d.talkMouths.length;
                }
                const mouthOpen = d.talkMouths[d.talkIdx];
                const info = getSpriteInfo(currentExpr, bob + sentimentShift);
                if (info) {
                    setupTransform(info, bob * 0.3, 1);
                    drawBase(info);
                    drawProceduralFace(info, currentExpr, mouthOpen);
                    drawBlink(info, tickBlink(dt, 0.7));
                    ctx.restore();
                }
                ...
                drawGlow(glowColor, gi, sentimentShift);
                
                const tilt = Math.sin(d.swayPhase * 0.8) * -3.5 - 2;
                const info = getSpriteInfo(currentExpr, sentimentShift);
                if (info) {
                    setupTransform(info, tilt, 1);
                    drawBase(info);
                    drawProceduralFace(info, currentExpr, 0);
                    drawBlink(info, tickBlink(dt));
                    ctx.restore();
                ...
                const tilt = Math.sin(d.swayPhase * 0.7) * 4 + 5;
                const info = getSpriteInfo('thinking', sentimentShift);
                if (info) {
                    setupTransform(info, tilt, 1);
                    drawBase(info);
                    drawProceduralFace(info, 'thinking', 0);
                    drawBlink(info, tickBlink(dt, 1.5));
                    ctx.restore();
                }
            }
